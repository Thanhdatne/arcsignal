...
export default async function AdminMarketPage({
  ...
  const isPendingReview = market.reviewStatus === "pending_review"
  const isApproved = market.reviewStatus === "approved"
    ...
    <main className="min-h-screen bg-transparent text-white">
      <div className="mx-auto max-w-5xl px-6 pt-32 pb-20">
        ...
        <p className="mt-5 max-w-3xl text-zinc-400">
          Review submitted markets, initialize Arc market IDs, submit oracle
          outcomes, and finalize settlement after the dispute window.
        </p>

        <div className="mt-14 rounded-3xl border border-zinc-900 bg-zinc-950/80 p-8">
          <div className="mb-8">
            <p className="mb-2 text-sm text-zinc-500">Market</p>

            <h2 className="text-3xl font-semibold">{market.title}</h2>
          </div>

          <div className="grid gap-4 md:grid-cols-3">
            <div className="rounded-2xl border border-zinc-900 bg-black/40 p-5">
              <p className="mb-3 text-sm text-zinc-500">Review</p>

              <h3
                className={`text-2xl font-semibold ${
                  isApproved ? "text-emerald-300" : "text-yellow-300"
                }`}
              >
            ...
            <div className="rounded-2xl border border-zinc-900 bg-black/40 p-5">
              <p className="mb-3 text-sm text-zinc-500">Arc Market ID</p>

              <h3 className="text-2xl font-semibold">
                {market.onchainId ?? "Not linked"}
              </h3>
            </div>

            <div className="rounded-2xl border border-zinc-900 bg-black/40 p-5">
              <p className="mb-3 text-sm text-zinc-500">Status</p>
          ...
          {isApproved && !market.onchainId ? (
            <form
              action={async () => {
                "use server"

                await linkMarketOnchain(market.id)
              }}
              className="mt-6"
            >
              <button
          ...
          <div className="mt-6 rounded-2xl border border-zinc-900 bg-black/40 p-5">
            ...
            <p className="leading-relaxed text-zinc-300">
              {market.resolution || "No resolution source provided."}
            </p>
          </div>
          ...
          {isApproved ? (
            ...
            <div className="mt-8 rounded-2xl border border-yellow-300/20 bg-yellow-300/5 p-5 text-sm text-yellow-300">
              Approve this market before initializing onchain settlement or
              submitting oracle resolution.
            </div>
          )}
          ...
          {isPendingResolution ? (
            <FinalizeSettlementButton
              marketId={market.id}
              disputeEndsAt={market.disputeEndsAt}
        ...
        <div className="mt-10 rounded-3xl border border-zinc-900 bg-zinc-950/80 p-8">
          <div className="mb-8 flex items-center justify-between">
            <div>
              <p className="mb-2 text-sm text-emerald-300">Activity</p>
            ...
            <p className="text-sm text-zinc-500">
              Review, oracle & treasury activity
            </p>
          </div>
